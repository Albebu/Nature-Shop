...
import { RabbitMQEventPublisher } from './infrastructure/driven/rabbitmq-event-publisher.js';
...
import { VerificationTokenRepositoryPrisma } from './infrastructure/driven/verification-token.repository.prisma.js';
...
export function createApp(options: CreateAppOptions = {}): Express {
  ...
  const verificationTokenRepository = new VerificationTokenRepositoryPrisma(prisma);
  ...
  // Event publisher — real adapter if RabbitMQ channel is available, noop otherwise
  const eventPublisher: EventPublisher = options.channel
    ? new RabbitMQEventPublisher(options.channel, loggerAdapter)
    : {
        publish: (): Promise<void> => {
          loggerAdapter.warn({}, 'EventPublisher not configured — event discarded');
